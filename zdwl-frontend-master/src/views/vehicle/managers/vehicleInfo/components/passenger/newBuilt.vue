...
import { State, PassengerFrom } from "@/api/vehicleInfo/types";
...
const form = reactive<PassengerFrom>({
  id: undefined,
  card: undefined,
  brand: "",
  ownership: "",
...
const rules = reactive<FormRules<PassengerFrom>>({
  card: [
    {
      required: true,
      message: "请输入车牌号码",
...
const getVehicleTractor = async () => {
  try {
    const { data } = await vehicleCar(form);
...
const editPuttractor = async () => {
  try {
    const { data } = await vehicleEdit(form, props.editFrom.id);
