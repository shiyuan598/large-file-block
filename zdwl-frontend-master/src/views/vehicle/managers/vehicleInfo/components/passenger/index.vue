...
const getStatList = async () => {
  state.loading = true;
  const { data } = await getTrailerPage(form, "/api/v1/vehicle/car/page");
  try {
...
const editClick = (index: any, row: any) => {
  emit("newBuilt", row, "passengerFrom");
};

// 删除
const handleDelete = async (index: any, ids: any) => {
  await deleteTractor(ids, "/api/v1/vehicle/car/");
  getStatList();
};
// 查询
const onSubmit = () => {
...
// 导出
const exportVehicle = () => {
  exportFile(form, "/api/v1/vehicle/car/_export");
};
defineExpose({
  getStatList,
  setList,
