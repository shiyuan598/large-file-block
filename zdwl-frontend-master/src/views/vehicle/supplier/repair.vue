  ...
  <div class="repairClass">
    <footer class="header__footer">
      <el-form :inline="true" :model="queryParams" class="demo-form-inline"  ref="queryFormRef" >
        <el-form-item label="修理厂名称">
          <el-input v-model="queryParams.keywords" placeholder="请输入修理厂名称" clearable />
        </el-form-item>
    ...
    <div style="padding: 10px 20px; width: 100%;">
      ...
      >
        ...
        <el-table-column prop="name" label="修理厂名称" width="300">
        ...
        <el-table-column prop="recordNumMonth" label="本月维修记录(次)" align="right"/>
        <el-table-column prop="useAmountMonth" label="本月维修金额(元)" align="right">
          <template #default="scope">
            <div v-if="scope.row.useAmountMonth > 200000">
              <span style="color:rgba(255, 77, 79, 1);">{{ scope.row.useAmountMonth }}</span>
            </div>
            <div v-else>
              <span>{{ scope.row.useAmountMonth }}</span>
            </div>
          </template>
        </el-table-column>
        ...
        <el-table-column prop="action" label="操作" width="180">
          <template #default="scope">
            <el-button text type="primary" @click="editClick(scope.$index, scope.row)">编辑</el-button>
            <el-button text type="primary" @click="handleDelete(scope.$index, scope.row)" v-if="scope.row.status == 0">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog
      ...
      :title="state.dialogAction=='new'?'新建修理厂':'编辑修理厂'"
      width="50%"
...
import { getRepairPage, updateRepair, addRepair, deleteRepair, exportSupplier, getTemplate, importSupplier} from "@/api/supplier/index";
...
const resetForm = () => {
  newForm.id = 0
  newForm.name = ''
  newForm.recordNumMonth = ''
  newForm.useAmountMonth = ''
...
const handleStatusChange = (val:any) => {
  updateRepair(val.id, val).then(res=>{
...
const newFormClick = () => {
  if(state.dialogAction == 'new'){
    ...
    updateRepair(newForm.id, newForm).then(res=>{
...
const importClick = () => {
  // router.push({ path: "/import", query: {type: "repair"} });
  emit('disableClick', true);
  emit('paramsFn', {importFn: state.apiFn, downFn: state.downFn, downloadType: "repair", title:"智能车管 - 供应商信息"})
}

const exportClick = () => {
  queryParams.supplierType = 'repair'
...
const deleteFormConfirm = () => {
  dialogDeleteVisible.value = false
  deleteRepair(state.deleteId).then(res=>{
...
const handleQuery = () => {
  loading.value = true;
  getRepairPage(queryParams).then(({ data }) => {
    state.tableData = []
