  ...
  <div class="trailerClass">
    <footer class="header__footer">
      <el-form :inline="true" :model="queryParams" class="demo-form-inline"  ref="queryFormRef" >
        <el-form-item label="租赁公司名称">
          <el-input v-model="queryParams.keywords" placeholder="请输入租赁公司名称" clearable />
        </el-form-item>
    ...
    <div style="padding: 10px 20px; width: 100%;">
      ...
      >
        ...
        <el-table-column prop="name" label="租赁公司名称" width="300">
        ...
        <el-table-column prop="useAmountMonth" label="本月累计金额(L)" align="right"/>
    ...
    <el-dialog
      ...
      :title="state.dialogAction=='new'?'新建租赁公司':'编辑租赁公司'"
      width="50%"
        ...
        <el-row :gutter="20">
          ...
          <el-col :span="12">
            <el-form-item label="联系人姓名" prop="contactName">
              <el-input v-model="newForm.contactName" placeholder="请输入联系人姓名" />
            </el-form-item>
          </el-col>
        </el-row>
        <el-row :gutter="20">
...
import { getLeasePage, updateLease, addLease, deleteLease, exportSupplier, getTemplate, importSupplier} from "@/api/supplier/index";
...
const getDisabled = computed(()=> (newForm.name==='' || newForm.contactName==='' || newForm.phone===''))
...
const handleStatusChange = (val:any) => {
  updateLease(val.id, val).then(res=>{
...
const newFormClick = () => {
  if(state.dialogAction == 'new'){
    ...
    updateLease(newForm.id, newForm).then(res=>{
...
const importClick = () => {
  // router.push({ path: "/import", query: {type: "lease"} });
  emit('disableClick', true);
  emit('paramsFn', {importFn: state.apiFn, downFn: state.downFn, downloadType: "lease", title:"智能车管 - 供应商信息"})
}

const exportClick = () => {
  queryParams.supplierType = 'lease'
...
const deleteFormConfirm = () => {
  dialogDeleteVisible.value = false
  deleteLease(state.deleteId).then(res=>{
...
const handleQuery = () => {
  loading.value = true;
  getLeasePage(queryParams).then(({ data }) => {
      state.tableData = []
