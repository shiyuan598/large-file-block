  ...
  <div class="parkClass">
    <footer class="header__footer">
      <el-form :inline="true" :model="queryParams" class="demo-form-inline"  ref="queryFormRef" >
        <el-form-item label="停车场名称">
          <el-input v-model="queryParams.keywords" placeholder="请输入停车场名称" clearable />
        </el-form-item>
    ...
    <div style="padding: 10px 20px; width: 100%;">
      ...
      >
        ...
        <el-table-column prop="name" label="停车场名称" width="300">
        ...
        <el-table-column prop="useAmountMonth" label="本月累计金额(元)" align="right"/>
    ...
    <el-dialog
      ...
      :title="state.dialogAction=='new'?'新建停车场':'编辑停车场'"
      width="50%"
...
import { getParkPage, updatePark, addPark, deletePark, exportSupplier, getTemplate, importSupplier} from "@/api/supplier/index";
...
const handleStatusChange = (val:any) => {
  updatePark(val.id, val).then(res=>{
...
const newFormClick = () => {
  if(state.dialogAction == 'new'){
    ...
    updatePark(newForm.id, newForm).then(res=>{
...
const importClick = () => {
  // router.push({ path: "/import", query: {type: "parkin"} });
  emit('disableClick', true);
  emit('paramsFn', {importFn: state.apiFn, downFn: state.downFn, downloadType: "parkin", title:"智能车管 - 供应商信息"})
}

const exportClick = () => {
  queryParams.supplierType = 'parkin'
...
const deleteFormConfirm = () => {
  dialogDeleteVisible.value = false
  deletePark(state.deleteId).then(res=>{
...
const handleQuery = () => {
  loading.value = true;
  getParkPage(queryParams).then(({ data }) => {
      state.tableData = []
