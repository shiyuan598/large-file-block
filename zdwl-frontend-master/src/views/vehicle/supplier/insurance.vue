  ...
  <div class="insuranceClass">
    <footer class="header__footer">
      <el-form :inline="true" :model="queryParams" class="demo-form-inline"  ref="queryFormRef" >
        <el-form-item label="保险公司名称">
          <el-input v-model="queryParams.keywords" placeholder="请输入保险公司名称" clearable />
        </el-form-item>
    ...
    <div style="padding: 10px 20px; width: 100%;">
      ...
      >
        ...
        <el-table-column prop="name" label="保险公司名称" width="300">
        ...
        <el-table-column prop="useAmountYear" label="年度累计金额(元)" align="right"/>
    ...
    <el-dialog
      ...
      :title="state.dialogAction=='new'?'新建保险公司':'编辑保险公司'"
      width="50%"
...
import { getInsurancePage, updateInsurance, addInsurance, deleteInsurance, exportSupplier, importSupplier, getTemplate} from "@/api/supplier/index";
...
const handleStatusChange = (val:any) => {
  updateInsurance(val.id, val).then(res=>{
...
const newFormClick = () => {
  if(state.dialogAction == 'new'){
    ...
    updateInsurance(newForm.id, newForm).then(res=>{
...
const deleteFormConfirm = () => {
  dialogDeleteVisible.value = false
  deleteInsurance(state.deleteId).then(res=>{
...
const handleQuery = () => {
  loading.value = true;
  getInsurancePage(queryParams).then(({ data }) => {
      state.tableData = []
