  ...
  <div class="etcClass">
    <footer class="header__footer">
      <el-form :inline="true"  :model="queryParams" class="demo-form-inline"  ref="queryFormRef" >
        <el-form-item label="ETC供应商名称">
          <el-input v-model="queryParams.keywords" placeholder="请输入ETC供应商名称" clearable />
        </el-form-item>
    ...
    <div style="padding: 10px 20px; width: 100%;">
      ...
      >
        ...
        <el-table-column prop="name" label="ETC供应商名称" width="300">
        ...
        <el-table-column prop="balance" label="剩余金额(元)" align="right">
          <template #default="scope">
            <div v-if="scope.row.balance < 100000">
              <span style="color:rgba(255, 77, 79, 1);">{{ scope.row.balance }}</span>
            </div>
            <div v-else>
    ...
    <el-dialog
      ...
      :title="state.dialogAction=='new'?'新建ETC供应商':'编辑ETC供应商'"
      width="50%"
...
import { getETCPage, updateETC, addETC, deleteETC, exportSupplier, getTemplate, importSupplier} from "@/api/supplier/index";
...
const newFormClick = () => {
  if(state.dialogAction == 'new'){
    ...
    updateETC(newForm.id, newForm).then(res=>{
...
const importClick = () => {
  // router.push({ path: "/import", query: {type: "etc"} });
  emit('disableClick', true);
  emit('paramsFn', {importFn: state.apiFn, downFn: state.downFn, downloadType: "etc", title:"智能车管 - 供应商信息"})
}

const exportClick = () => {
  queryParams.supplierType = 'etc'
...
const deleteFormConfirm = () => {
  dialogDeleteVisible.value = false
  deleteETC(state.deleteId).then(res=>{
...
const handleQuery = () => {
  loading.value = true;
  getETCPage(queryParams).then(({ data }) => {
      state.tableData = []
