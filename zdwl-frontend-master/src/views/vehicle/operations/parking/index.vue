      ...
      <el-container>
        <!-- <el-header style="background-color: white;position: relative;">
          <span style="font-size: 12px;color: rgb(181, 181, 181);position: absolute;top: 35%;">当前位置：智能车管
            ...
            <span style="padding: 10px;">-</span>
            <span style="color:black;">固定停车</span>
          </span>
        </el-header> -->
        <el-header style="background-color: white;display: flex;justify-content: flex-start;height:56px;">
          ...
          <el-breadcrumb separator="-" style="display: flex;align-items: center;">
            ...
            <el-breadcrumb-item style="font-size: 14px;font-family: PingFangSC, PingFang SC;color: #323233;" v-if="!importFlg">固定停车</el-breadcrumb-item>
            <el-breadcrumb-item :to="{ path: '/vehicle/operations/parking' }" @click="returnList" v-if="importFlg">固定停车</el-breadcrumb-item>
                    ...
                    <el-row class="row-bg" justify="space-between">
                      <el-col :span="6">
                        <el-form-item label="停车场">
                          <el-select v-model="queryParams.supplierCode" size="default" placeholder="请选择停车场名称" style="align-items: center;width: 100%;" clearable>
                            <el-option
                              v-for="item in supplierParkOptions"
                              :key="item.id"
                              :label="item.name"
                              :value="item.id"
                            />
      ...
      >
        <el-form  :model="newForm" :rules="newRules" label-position="left" label-width="100px" ref="newFormRef" class="newFormClass">
          ...
          <el-row :gutter="20">
            ...
            <el-col :span="12">
              <el-form-item label="停车场" prop="supplierParking">
                <el-select v-model="newForm.supplierParking" size="default" placeholder="请选择停车场名称" style="align-items: center;width: 100%;" @change="supplierParkClick" clearable>
                  <el-option
                    v-for="item in supplierParkOptions"
                    :key="item.id"
                    :label="item.name"
...
import { ParkingForm, ParkingQuery, ParkingPageVO } from '@/api/operations/parking/types';
import { getParkingPage, updateParking, addParking, deleteParking, exportParking, getParkingStat,importParking, getParkingTemplate} from "@/api/operations/parking/index";
...
const queryParams = reactive<ParkingQuery>({
  pageNum: 1,
  pageSize: 10,
});
const state = reactive({
  ...
  endTime: '',
  apiFn: importParking,
  downFn: getParkingTemplate,
  title: '智能车管 - 智慧运营 - 固定停车',
})
...
const getDisabled = computed(()=> (newForm.card==='' || newForm.startTime==='' || newForm.endTime==='' || newForm.amount===''))
...
const newFormClick = () => {
  if(state.dialogAction == 'new'){
    addParking(newForm).then((res: any)=>{
    ...
    updateParking(newForm.id, newForm).then((res: any)=>{
...
const exportClick = () => {
  exportParking(queryParams).then((response)=>{
...
const deleteFormConfirm = () => {
  dialogDeleteVisible.value = false
  deleteParking(state.deleteId).then((res: any)=>{
...
const handleQuery = () => {
  loading.value = true;
  getParkingPage(queryParams).then((rs :any) => {
      state.tableData = []
...
const supplierParkClick = (obj: any) => {
  newForm.supplierParkingCode = supplierParkOptions.value.find(item=> item.name === obj).id
}

const getParkingStatInfo = () => {
  getParkingStat(queryParams).then(rs=>{
