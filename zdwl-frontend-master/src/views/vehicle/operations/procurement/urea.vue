  ...
  <div class="ureaClass">
    ...
    <div style="padding: 10px 20px; width: 100%">
      ...
      >
        ...
        <el-table-column prop="purNum" label="采购数量(KG)" align="right" />
        <el-table-column
          prop="purPriceLitre"
          label="采购单价(元/KG)"
          align="right"
      ...
      >
        ...
        <el-row >
          <el-form-item label="项目名称" prop="projectCode">
              <el-select @change="changeProject" v-model="newForm.projectCode" size="default" placeholder="请选择项目名称" style="align-items: center;width: 100%;" clearable>
                <el-option
                  v-for="item in props.projectOption"
                  :key="item.value"
                  :label="item.label"
...
const queryParams = reactive<PurchaseUreaQuery>({
  pageNum: 1,
  pageSize: 10,
});
...
const changeProject = (val:string)=>{
  newForm.project = props.projectOption.filter((n:any)=>n.value==val)[0]?.label;
}
...
const importClick = () => {
  ...
  emit("paramsFn", {
    ...
    downFn: state.downFn,
    downloadType: "",
    title: "智能车管 - 采购管理 - 尿素采购",
  });
};

const exportClick = () => {
  exportUrea(queryParams)
    .then((response) => {
...
const getUreaStatInfo = () => {
  getUreaStat(queryParams).then((rs) => {
