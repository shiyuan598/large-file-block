  ...
  <div class="dieselClass">
    ...
    <el-row class="mb-4">
      ...
      >
        ...
        <span class="span-color">采购数量</span
        ><span style="padding: 5px" class="text-span">{{ numSum }}</span
        ><span class="span-color">L</span>
        <span style="padding: 0 20px">|</span>
        <span class="span-color">采购金额</span
        ><span style="padding: 5px" class="text-span">{{ amountSum }}</span
        ><span class="span-color">元</span>
        <span style="padding: 0 20px">|</span>
        <span class="span-color">平均每升采购价格</span
        ><span style="padding: 5px" class="text-span">{{
          numSum&&calculate(amountSum, numSum, "/")?.toFixed(2)
        }}</span
        ><span class="span-color">元</span>
      </div>
    </el-row>
    <div style="padding: 10px 20px; width: 100%">
      ...
      >
        ...
        <el-table-column prop="supplierFuel" label="加油站名称" />
        <el-table-column prop="project" label="项目名称" />
        <el-table-column prop="purNum" label="采购数量(L)" align="right" />
        <el-table-column
          prop="purPriceLitre"
          label="采购单价(元/L)"
          align="right"
    ...
    >
      ...
      >
        <el-row :gutter="50" justify="space-evenly">
          ...
          <el-col :span="8">
            <el-form-item label="加油站名称" prop="supplierFuel">
              <el-select
                v-model="newForm.supplierFuel"
                size="default"
                placeholder="请选择加油站名称"
                style="align-items: center; width: 100%"
                @change="supplierFuelChange"
                filterable
                remote
                reserve-keyword
        ...
        <el-row justify="space-evenly" :gutter="50">
          <el-col :span="8">
            <el-form-item label="采购量" prop="purNum">
              <el-input
                v-model="newForm.purNum"
                placeholder="请输入采购量"
                style="width: 100%"
              />
            </el-form-item>
          ...
          <el-col :span="8">
            <el-form-item label="采购单价" prop="purPriceLitre">
              <div style="display: flex; flex-wrap: nowrap">
                <el-input
                  v-model="newForm.purPriceLitre"
                  placeholder="请输入采购单价"
                  style="width: 100%"
                >
        ...
        <el-row justify="space-evenly" :gutter="50">
          <el-col :span="8">
            <el-form-item label="采购单价" prop="purPriceTon">
              <div style="display: flex; flex-wrap: nowrap">
                <el-input
                  v-model="newForm.purPriceTon"
                  placeholder="请输入采购单价"
                  style="width: 100%"
                >
        ...
        <h3>当日参考价格</h3>
        <el-row
          class="row-bg referenceClass"
          justify="space-evenly"
          :gutter="50"
        >
          <el-col :span="8">
            <el-form-item label="中石化河南挂牌价" prop="refPriceHenan">
              <el-input v-model="newForm.refPriceHenan" style="width: 100%">
                <template #append>元/升</template>
              </el-input>
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item label="中石化安徽挂牌价" prop="refPriceAnhui">
              <el-input v-model="newForm.refPriceAnhui" style="width: 100%">
                <template #append>元/升</template>
              </el-input>
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item label="中石化江苏挂牌价" prop="refPriceJiangsu">
              <el-input v-model="newForm.refPriceJiangsu" style="width: 100%">
                <template #append>元/升</template>
              </el-input>
            </el-form-item>
          </el-col>
        ...
        >
          <el-col :span="8">
            <el-form-item label="中石化广东挂牌价" prop="refPriceGuangdong">
              <el-input v-model="newForm.refPriceGuangdong" style="width: 100%">
                <template #append>元/升</template>
              </el-input>
            </el-form-item>
          </el-col>
          <el-col :span="16" class="dfClass">
            <el-form-item
              label="东方财富网单日柴油批发价"
              prop="refPriceDongfang"
            >
              <el-input v-model="newForm.refPriceDongfang" style="width: 100%">
                <template #append>元/吨</template>
              </el-input>
            </el-form-item>
          </el-col>
...
const queryParams = reactive<PurchaseFuelQuery>({
  pageNum: 1,
  pageSize: 10,
});
...
const getDisabled = computed(
  () =>
    newForm.business === "" ||
    newForm.supplierFuel === "" ||
    newForm.purTime === "" ||
    newForm.purNum === "" ||
    newForm.purAmount === "" ||
    newForm.purPriceLitre === "" ||
    newForm.projectCode === ""
);
...
interface RuleForm {
  ...
  business: string;
  businessCode: number;
  refPriceHenan: string;
  refPriceAnhui: string;
  refPriceJiangsu: string;
  refPriceGuangdong: string;
  refPriceDongfang: string;
  projectCode: string;
  project: string;
}
...
const newRules = reactive<FormRules<RuleForm>>({
  ...
  supplierFuel: [
    { required: true, message: "请选择加油站名称", trigger: "blur" },
  ],
  purTime: [{ required: true, message: "请选择采购日期", trigger: "blur" }],
  purNum: [{ required: true, message: "请输入采购量", trigger: "blur" }],
...
const resetForm = () => {
  ...
  newForm.refPriceHenan = "";
  newForm.refPriceAnhui = "";
  newForm.refPriceJiangsu = "";
  newForm.refPriceGuangdong = "";
  newForm.refPriceDongfang = "";
  newForm.project = "";
  newForm.projectCode = "";
  fileList.value = [];
};
...
const supplierFuelChange = (obj: any) => {
  newForm.supplierFuelCode = fuelOptions.value.find(
    (item) => item.name === obj
  ).id;
};
...
const remoteMethod = (query: string) => {
  getFuelInfo({ status: 1, keywords: query }).then((rs) => {
    fuelOptions.value = [];
...
const getFuelStatInfo = () => {
  getFuelStat(queryParams).then((rs) => {
    numSum.value = rs.data.numSum;
    amountSum.value = rs.data.amountSum;
  });
};
