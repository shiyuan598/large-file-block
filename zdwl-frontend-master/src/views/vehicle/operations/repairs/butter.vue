        ...
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="维修日期" prop="repairTime">
              <el-date-picker v-model="newForm.repairTime" type="date" placeholder="请选择维修日期" style="width: 100%;" date-format="YYYY/MM/DD ddd"
                              value-format="YYYY-MM-DD"/>
            </el-form-item>
          </el-col>
          <el-col :span="12">
...
import { RepairButterForm, RepairButterQuery, RepairButterPageVO } from "@/api/operations/repairs/types";
import { getButterPage, updateButter, addButter, deleteButter, exportButter, getButterStat} from "@/api/operations/repairs/index";
...
import { getButterTemplate, importButter} from "@/api/operations/repairs/index";
const multipleSelection = ref([])
const total = ref(0);
const dialogNewVisible = ref(false)
const dialogDeleteVisible = ref(false)
const queryFormRef = ref(ElForm);
const queryParams = reactive<RepairButterQuery>({
  pageNum: 1,
  pageSize: 10,
});
...
const getDisabled = computed(()=> (newForm.card==='' || newForm.repairTime==='' || newForm.supplierRepairCode==='' || newForm.amount===''))
...
const handleStatusChange = (val:any) => {
  updateButter(val.id, val).then(_res=>{
...
const newFormClick = () => {
  if(state.dialogAction == 'new'){
    ...
    updateButter(newForm.id, newForm).then(_res=>{
...
const exportClick = () => {
  exportButter(queryParams).then(response=>{
...
const deleteFormConfirm = () => {
  dialogDeleteVisible.value = false
  deleteButter(state.deleteId).then(_res=>{
...
const handleQuery = () => {
  loading.value = true;
  getButterPage(queryParams).then(({ data }) => {
      state.tableData = []
...
const getButterStatInfo = () => {
  getButterStat(queryParams).then(rs=>{
