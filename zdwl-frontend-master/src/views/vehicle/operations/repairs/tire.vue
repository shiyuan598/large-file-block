  ...
  <div class="butterClass">
    <footer class="header__footer">
      <el-form :model="queryParams" class="demo-form-inline"  ref="queryFormRef" style="margin-bottom: 0px;">
        ...
        <el-row class="row-bg" justify="space-between">
          ...
          <el-col :span="6">
            <el-form-item label="轮胎维修类型">
              <el-select v-model="queryParams.tireRepairType" size="default" placeholder="请选择轮胎维修类型" style="align-items: center;width: 100%;" clearable>
                <el-option
    ...
    <div style="padding: 10px 20px; width: 100%;">
      ...
      >
        ...
        <el-table-column prop="tireRepairType" label="轮胎维修类型"/>
        <el-table-column prop="amount" label="维修金额（元）" align="right"/>
        <el-table-column prop="tireLocationCode" label="轮胎位置"/>
    ...
    >
      <el-form  :model="newForm" :rules="newRules" label-position="left" label-width="100px" ref="newFormRef" class="newFormClass">
        ...
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="维修日期" prop="repairTime">
              <el-date-picker v-model="newForm.repairTime" type="date" placeholder="请选择维修日期" style="width: 100%;" value-format="YYYY-MM-DD"/>
            </el-form-item>
          </el-col>
          <el-col :span="12">
        ...
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="类型" prop="tireRepairType">
              <el-select v-model="newForm.tireRepairType" size="default" placeholder="请选择类型" style="align-items: center;width: 100%;" clearable>
                <el-option
                  v-for="item in repairTypeOptions"
                  :key="item.id"
                  :label="item.label"
        ...
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="轮胎位置" prop="tireLocationCode">
              <el-input v-model="newForm.tireLocationCode" placeholder="请输入轮胎位置" style="width: 100%;"/>
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="品牌型号" prop="tireBrand">
              <el-input v-model="newForm.tireBrand" placeholder="请输入品牌型号" style="width: 100%;"/>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="花纹" prop="tireFigure">
              <el-input v-model="newForm.tireFigure" placeholder="请输入花纹" style="width: 100%;"/>
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="生产日期" prop="tireTime">
              <el-date-picker v-model="newForm.tireTime" type="date" placeholder="请选择生产日期" style="width: 100%;" value-format="YYYY-MM-DD" date-format="YYYY/MM/DD ddd"/>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row :gutter="20">
...
import { RepairTireForm, RepairTireQuery, RepairTirePageVO } from "@/api/operations/repairs/types";
import { getTirePage, updateTire, addTire, deleteTire, exportTire, getTireStat} from "@/api/operations/repairs/index";
...
import { getTireTemplate, importTire} from "@/api/operations/repairs/index";
const multipleSelection = ref([])
const total = ref(0);
const dialogNewVisible = ref(false)
const dialogDeleteVisible = ref(false)
const queryFormRef = ref(ElForm);
const queryParams = reactive<RepairTireQuery>({
  pageNum: 1,
  pageSize: 10,
});
...
const getDisabled = computed(()=> (newForm.business=='' || newForm.card=='' || newForm.repairTime=='' || newForm.supplierRepairCode=='' || newForm.tireRepairType=='' || newForm.tireLocationCode=='' || newForm.amount==''))
...
const newRules = reactive<FormRules<RuleForm>>({
  ...
  tireLocationCode: [
    { required: true, message: '请输入轮胎位置', trigger: 'blur' },
  ]
})
...
const resetForm = () => {
  ...
  newForm.card = ''
  newForm.repairTime = ''
  newForm.tireRepairType = ''
  newForm.tireLocationCode = ''
  newForm.tireBrand = ''
  newForm.tireFigure = ''
  newForm.tireTime = ''
...
const onReset = () => {
  ...
  queryParams.tireRepairType = ''
  handleQuery();
}

const handleStatusChange = (val:any) => {
  updateTire(val.id, val).then(_res=>{
...
const newFormClick = () => {
  if(state.dialogAction == 'new'){
    ...
    updateTire(newForm.id, newForm).then(_res=>{
...
const exportClick = () => {
  exportTire(queryParams).then(response=>{
...
const deleteFormConfirm = () => {
  dialogDeleteVisible.value = false
  deleteTire(state.deleteId).then(_res=>{
...
const handleQuery = () => {
  loading.value = true;
  getTirePage(queryParams).then(({ data }) => {
      state.tableData = []
...
const getTireStatInfo = () => {
  getTireStat(queryParams).then(rs=>{
