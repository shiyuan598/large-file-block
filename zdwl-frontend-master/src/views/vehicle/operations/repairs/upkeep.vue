  ...
  <div class="butterClass">
        ...
        <el-row class="row-bg" justify="space-between">
          <el-col :span="6">
            <el-form-item label="保养位置">
              <el-select v-model="queryParams.upkeepType" size="default" placeholder="请选择保养位置" style="align-items: center;width: 100%;" clearable>
                <el-option
                  v-for="item in upkeepTypeOptions"
                  :key="item.value"
                  :label="item.label"
                  :value="item.label"
                />
    ...
    <div style="padding: 10px 20px; width: 100%;">
      ...
      >
        ...
        <el-table-column prop="repairTime" label="保养日期" sortable>
          <template #default="scope">
              {{dayjs(scope.row.repairTime).format('YYYY-MM-DD') }}
          </template>
        </el-table-column>
        <el-table-column prop="supplierRepair" label="维修厂名称"/>
        <el-table-column prop="amount" label="保养金额（元）" align="right"/>
        <el-table-column prop="upkeepType" label="保养位置"/>
        <el-table-column prop="content" label="保养项目"/>
        <el-table-column prop="mileage" label="本次保养公里数" align="right"/>
        <el-table-column prop="nextMileage" label="下次保养公里数" align="right"/>
    ...
    >
      <el-form  :model="newForm" :rules="newRules" label-position="left" label-width="130px" ref="newFormRef" class="newFormClass">
        <el-row :gutter="20">
          <el-col :span="12">
        ...
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="保养日期" prop="repairTime">
              <el-date-picker v-model="newForm.repairTime" type="date" placeholder="请选择保养日期" style="width: 100%;" value-format="YYYY-MM-DD"/>
            </el-form-item>
          </el-col>
          <el-col :span="12">
        ...
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="保养位置" prop="upkeepType">
              <el-select v-model="newForm.upkeepType" size="default" placeholder="请选择保养位置" style="align-items: center;width: 100%;" @change="upkeepTypeChange" clearable>
                <el-option
                  v-for="item in upkeepTypeOptions"
                  :key="item.value"
                  :label="item.label"
          ...
          <el-col :span="12">
            <el-form-item label="保养项目" prop="content">
              <el-input v-model="newForm.content" placeholder="请输入保养项目" style="width: 100%;" />
            </el-form-item>
          </el-col>
        </el-row>
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="保养金额" prop="amount">
          ...
          <el-col :span="12">
            <el-form-item label="本次保养公里数" prop="mileage">
              <el-input v-model="newForm.mileage" placeholder="请输入本次保养公里数" style="width: 100%;">
        ...
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="下次保养公里数" prop="nextMileage">
              <el-input v-model="newForm.nextMileage" placeholder="请输入下次保养公里数" style="width: 100%;">
                <template #append>公里</template>
              </el-input>
            </el-form-item>
          </el-col>
...
import { RepairUpkeepForm, RepairUpkeepQuery, RepairUpkeepPageVO } from "@/api/operations/repairs/types";
import { getUpkeepPage, updateUpkeep, addUpkeep, deleteUpkeep, exportUpkeep, getUpkeepStat} from "@/api/operations/repairs/index";
...
import { getUpkeepTemplate, importUpkeep} from "@/api/operations/repairs/index";
const multipleSelection = ref([])
const total = ref(0);
const dialogNewVisible = ref(false)
const dialogDeleteVisible = ref(false)
const queryFormRef = ref(ElForm);
const queryParams = reactive<RepairUpkeepQuery>({
  pageNum: 1,
  pageSize: 10,
});

const upkeepTypeOptions = ref([] as any []);
...
const state = reactive({
  ...
  deleteId: '',
  apiFn: importUpkeep,
  downFn: getUpkeepTemplate,
  time: ''
})
const businessObj = ref(null)
const upkeepTypeObj = ref(null)
...
const getDisabled = computed(()=> (newForm.business==='' || newForm.card==='' || newForm.repairTime==='' || newForm.supplierRepairCode==='' || newForm.upkeepType==='' || newForm.amount===''))
...
const newRules = reactive<FormRules<RuleForm>>({
  ...
  upkeepType: [
    { required: true, message: '请选择保养位置', trigger: 'blur' },
  ],
})
...
const onReset = () => {
  ...
  queryParams.endTime = ''
  queryParams.upkeepType = ''
  queryParams.pageNum = 1;
  handleQuery();
}

const handleStatusChange = (val:any) => {
  updateUpkeep(val.id, val).then(_res=>{
...
const newFormClick = () => {
  if(state.dialogAction == 'new'){
    ...
    updateUpkeep(newForm.id, newForm).then(_res=>{
...
const upkeepTypeChange = (obj:any) => {
  newForm.upkeepType = obj.label
  newForm.upkeepTypeCode = obj.value
}
...
const exportClick = () => {
  exportUpkeep(queryParams).then(response=>{
...
const deleteFormConfirm = () => {
  dialogDeleteVisible.value = false
  deleteUpkeep(state.deleteId).then(_res=>{
...
const handleQuery = () => {
  loading.value = true;
  getUpkeepPage(queryParams).then(({ data }) => {
      state.tableData = []
...
const getUpkeepStatInfo = () => {
  getUpkeepStat(queryParams).then(rs=>{
...
onMounted(async () => {
  ...
  await getBusinessList('upkeep_type', upkeepTypeObj);
  Object.assign(upkeepTypeOptions.value, upkeepTypeObj)
