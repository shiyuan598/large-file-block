...
const mapList = reactive(props.mapListType === 'line'? props.gpsList : interList.value.gpsList);
onMounted(async () => {
  lodeMap();
});
...
const reloadMarker = async () => {
  if (Object.keys(maps.map).length !== 0) {
    ...
    if(mapList.length>0){
      console.log(interList.value.gpsList)
      console.log(mapList)
      setMaker();
    }
  }
...
const lodeMap = () => {
  AMapLoader.load({
    ...
    .then(async (AMap) => {
      maps.AMap = AMap;
     await newMap();
      // if (props.gpsList.length > 0) {
      //   setPolyline(props.gpsList);
      // }
...
const loadGpsMaker = () => {
  ...
  gpsMaker.value = new maps.AMap.Marker({
    // map: maps.map,
    // position: position,
    // position: [120.6520657321622, 31.42090114400731],
...
const setMaker = () => {
  ...
  const endMarker = new maps.AMap.Marker({
    position: new maps.AMap.LngLat(
      mapList[mapList.length - 1][0],
      mapList[mapList.length - 1][1]
    ),
    icon: endIcon,
    map: maps.map,
    offset: new maps.AMap.Pixel(-13, -30),
  ...
  mapList.map((n: any, index: number) => {
    if (index === 0 || index === mapList.length - 1) return;
